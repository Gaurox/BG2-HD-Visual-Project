...
function New-FakeGame([string]$Name,[string]$RendererIni=''){
    ...
    $script:fixtureSources[$game]=$steam;$game
}
function Snapshot([string]$Game){[ordered]@{baldur=Get-Hash (Join-Path $Game 'Baldur.exe');source_baldur=Get-Hash (Join-Path $script:fixtureSources[$Game] 'Baldur.exe');loader_ini=Get-Hash (Join-Path $Game 'InfinityLoader.ini');renderer_ini=Get-Hash (Join-Path $Game 'InfinityEngine-Enhancer.ini');renderer_files=Renderer-Snapshot $Game}}
function Assert-Baseline([string]$Game,[object]$Before,[string]$Desktop,[string]$Label){
    ...
    Require ((Get-Hash (Join-Path $script:fixtureSources[$Game] 'Baldur.exe'))-eq$Before.source_baldur) "$Label : source Steam modifiee"
...
function Assert-Removed([string]$Game,[object]$Before,[string]$Desktop,[string]$Label){
    Require ((Get-Hash (Join-Path $Game 'Baldur.exe'))-eq$loaderHash) "$Label : shim EEex non conserve"
    Require ((Get-Hash (Join-Path $Game 'BaldurReal.exe'))-eq$Before.baldur) "$Label : executable officiel non preserve"
    Require ((Get-Hash (Join-Path $script:fixtureSources[$Game] 'Baldur.exe'))-eq$Before.source_baldur) "$Label : source Steam modifiee"
    $loaderIni=Get-Content -LiteralPath (Join-Path $Game 'InfinityLoader.ini') -Raw
    Require ($loaderIni-match'(?im)^\s*ExeNames\s*=\s*BaldurReal\.exe' -and $loaderIni-match'(?im)^\s*ExeSwitchAlias\s*=\s*BaldurReal\.exe:Baldur\.exe') "$Label : routage EEex non conserve"
    Require ((Get-Hash (Join-Path $Game 'InfinityEngine-Enhancer.ini'))-eq$Before.renderer_ini) "$Label : INI renderer non restaure"
    foreach($file in $rendererRuntimeFiles){$path=Join-Path $Game $file.path.Replace('/','\');$actual=if(Test-Path -LiteralPath $path -PathType Leaf){Get-Hash $path}else{$null};Require ($actual-eq$Before.renderer_files.($file.path)) "$Label : renderer non restaure : $($file.path)"}
    Require ((Get-ChildItem -LiteralPath $Desktop -File -ErrorAction SilentlyContinue).Count-eq0) "$Label : raccourci BG2HD subsiste"
    $state=Get-Content -LiteralPath (Join-Path $Game 'bg2hd/state/steam-launcher.json') -Raw|ConvertFrom-Json;Require ($state.phase-eq'eeex-retained') "$Label : phase EEex conserve absente"
}
...
try{
    ...
    $repair=New-FakeGame 'repair';$desktop=Join-Path $temp 'desk-repair';New-Item -ItemType Directory -Path $desktop -Force|Out-Null;$before=Snapshot $repair;Require ((Invoke-Helper Install $repair $desktop)-eq0) 'Installation de reparation echouee.';$shortcut=Join-Path $desktop "Baldur's Gate II Enhanced Edition - HD.lnk";Remove-Item -LiteralPath $shortcut;Copy-Required $officialSource (Join-Path $repair 'Baldur.exe');Require ((Invoke-Helper Repair $repair $desktop)-eq0) 'Repair apres verification Steam echoue.';Require (Test-Path -LiteralPath $shortcut) 'Repair n a pas recree le raccourci.';[IO.File]::WriteAllBytes((Join-Path $repair 'Baldur.exe'),[byte[]](9,8,7,6));Require ((Invoke-Helper Repair $repair $desktop)-ne0) 'Repair a accepte un executable inconnu.';Require ((Invoke-Helper Uninstall $repair $desktop)-ne0) 'Uninstall a accepte un executable inconnu.';Require ((Get-Hash (Join-Path $repair 'BaldurReal.exe'))-eq$before.baldur) 'Refus sur executable inconnu a modifie BaldurReal.exe.'
    ...
    Require ((Invoke-Helper Uninstall $custom $desktop)-eq0) 'Uninstall avec INI externe modifie echoue.';Require ((Get-Hash (Join-Path $custom 'Baldur.exe'))-eq$loaderHash) 'Uninstall n a pas conserve le shim EEex.';Require ((Get-Hash (Join-Path $custom 'BaldurReal.exe'))-eq$customBefore.baldur) 'Uninstall n a pas preserve l executable officiel.';$restored=[Text.Encoding]::Unicode.GetString([IO.File]::ReadAllBytes($iniPath));Require ($restored-match'UserValue = user-change') 'Modification INI externe perdue.';Require ($restored-match'VerboseLogs = true ; old') 'Cle possedee non restauree.';Require (Test-Path -LiteralPath $userShortcut) 'Raccourci utilisateur retire.';Require (-not(Test-Path -LiteralPath $ownedShortcut)) 'Raccourci BG2HD subsiste.'
    'PHASE5A_IN_PLACE_CORE_MATRIX=PASSED'
