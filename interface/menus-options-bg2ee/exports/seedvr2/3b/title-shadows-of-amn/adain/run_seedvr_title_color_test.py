"""Generate an isolated SeedVR2 3B title comparison with AdaIN color transfer."""
...
def main() -> None:
    ...
    client.preflight()
    uploaded = client.upload(intermediate, "BG2_Upscale/exports-seedvr3b-title-adain")
    for scale in (2, 4):
        output = ROOT / f"x{scale}" / f"TITLE-frame-00-seedvr2-3b-adain-x{scale}.png"
        if output.exists():
        ...
        prompt["9"]["inputs"]["filename_prefix"] = f"BG2_Upscale/exports-seedvr3b-title-adain/TITLE-frame-00-x{scale}"
