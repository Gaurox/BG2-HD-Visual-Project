...
$testConfig = Join-Path $PSScriptRoot 'InfinityEngine-Enhancer-x4-topaz-recovery-v2-d50.ini'
$bigLogo = Join-Path $PSScriptRoot 'assets\BIGLOGO-MOS0017-x4.dxt5'
$assets = @(
    @{ Name = 'BIGLOGO-MOS0017-x4.dxt5'; Source = $bigLogo; Bytes = 16777216 },
    @{ Name = 'MAINMENU-MOS0181-x4.dxt5'; Source = (Join-Path $PSScriptRoot 'assets\MAINMENU-MOS0181-x4.dxt5'); Bytes = 16777216 },
    @{ Name = 'MAINMENU-MOS0257-x4.dxt5'; Source = (Join-Path $PSScriptRoot 'assets\MAINMENU-MOS0257-x4.dxt5'); Bytes = 16777216 },
    @{ Name = 'MAINMENU-MOS0258-x4.dxt5'; Source = (Join-Path $PSScriptRoot 'assets\MAINMENU-MOS0258-x4.dxt5'); Bytes = 16777216 },
    @{ Name = 'MAINMENU-MOS0261-x4.dxt5'; Source = (Join-Path $PSScriptRoot 'assets\MAINMENU-MOS0261-x4.dxt5'); Bytes = 16777216 },
    @{ Name = 'MAINMENU-MOS0262-x4.dxt5'; Source = (Join-Path $PSScriptRoot 'assets\MAINMENU-MOS0262-x4.dxt5'); Bytes = 4194304 },
    @{ Name = 'MAINMENU-MOS0265-x4.dxt5'; Source = (Join-Path $PSScriptRoot 'assets\MAINMENU-MOS0265-x4.dxt5'); Bytes = 4194304 },
    @{ Name = 'MAINMENU-MOS0266-x4.dxt5'; Source = (Join-Path $PSScriptRoot 'assets\MAINMENU-MOS0266-x4.dxt5'); Bytes = 4194304 }
)
foreach ($required in @($builtDll, $testConfig, $bigLogo) + $assets.Source) {
...
Copy-Item -LiteralPath $testConfig -Destination (Join-Path $gameRootPath 'InfinityEngine-Enhancer.ini') -Force
New-Item -ItemType Directory -Path $destinationRoot -Force | Out-Null
foreach ($asset in $assets) {
    Copy-Item -LiteralPath $asset.Source -Destination (Join-Path $destinationRoot $asset.Name) -Force
}
Write-Host "Test menu x4 Topaz Recover v2 Detail 50 installe. Sauvegarde creee : $backup"
