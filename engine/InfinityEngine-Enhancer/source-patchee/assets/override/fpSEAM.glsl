...
void main()
{
	...
	// Liquid mode of the WED cell under this fragment (R8: 0..6 -> 0..6/255).
	float cellMode = 0.0;
	if (uIeeEnabled > 0.5)
	{
	...
	if (waterMask > 0.02)
	{
		...
		else if (cellMode > 3.5 && cellMode < 4.5)
		{
			// Sewage uses the authored WTSEW animation but must read as a dark,
			// dirty floor-adjacent liquid rather than clean reflective water.
			deepColor = ieeSrgbToLinear(vec3(0.10, 0.075, 0.03));
			shallowColor = ieeSrgbToLinear(vec3(0.24, 0.19, 0.075));
