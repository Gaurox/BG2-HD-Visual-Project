...
struct AreaGpuSnapshot {
  ...
  std::size_t paletteTintPixels{};
  std::size_t paletteTintTiles{};
  std::vector<PvrzTintCandidate> pvrzTintCandidates;
  std::optional<AreaFallbackTint> fallbackTint;
...
constexpr int kMaxTintPageDimension = 4096;

void queue_area_gpu_snapshot(game::AreaCellTexture texture, std::array<float, 3> waterTint,
                             std::size_t paletteTintPixels = 0,
                             std::size_t paletteTintTiles = 0,
                             std::vector<PvrzTintCandidate> pvrzTintCandidates = {},
                             std::optional<AreaFallbackTint> fallbackTint = std::nullopt) {
  auto snapshot = std::make_shared<AreaGpuSnapshot>();
  snapshot->texture = std::move(texture);
  snapshot->waterTint = waterTint;
  snapshot->paletteTintPixels = paletteTintPixels;
  snapshot->paletteTintTiles = paletteTintTiles;
  snapshot->pvrzTintCandidates = std::move(pvrzTintCandidates);
  snapshot->fallbackTint = std::move(fallbackTint);
...
bool read_pvrz_tint_candidate(const void* tileWrapper, std::uint32_t expectedTileIndex,
                              std::string_view expectedTilesetResref,
                              PvrzTintCandidate& out) noexcept {
  out = {};
  game::CResTile tile{};
  if (!tileWrapper || !core::safe_read(tileWrapper, tile) || !tile.tis || !tile.pvr ||
      tile.tileIndex < 0) {
    return false;
  }

  game::CResTileSet tileset{};
  if (!core::safe_read(tile.tis, tileset) || !tileset.baseclass_0.pData ||
      tileset.baseclass_0.nSize != sizeof(game::PVRZTileEntry) ||
      static_cast<std::uint32_t>(tile.tileIndex) >= tileset.baseclass_0.nCount) {
    return false;
  }

  game::ResrefBuffer actualTilesetResref{};
  if (!game::read_runtime_resref(tileset.baseclass_0.resref, actualTilesetResref) ||
      !game::matches_tis_tile_identity(tile.tileIndex,
                                       game::resref_view(actualTilesetResref),
                                       expectedTileIndex, expectedTilesetResref)) {
    return false;
  }

  game::TileInfo tileInfo{};
  tileInfo.table = static_cast<const game::PVRZTileEntry*>(tileset.baseclass_0.pData);
  tileInfo.tileCount = tileset.baseclass_0.nCount;
  game::PVRZTileEntry entry{};
  if (!game::read_tis_tile_entry(tileInfo, static_cast<std::uint32_t>(tile.tileIndex), entry) ||
      entry.page < 0) {
    return false;
  }

  game::CResPVR pvr{};
  if (!core::safe_read(tile.pvr, pvr) || pvr.texture <= 0 || pvr.size.cx <= 0 ||
      pvr.size.cy <= 0 || pvr.size.cx > kMaxTintPageDimension ||
      pvr.size.cy > kMaxTintPageDimension) {
    return false;
  }

  game::ResrefBuffer actualPageResref{};
  if (!game::read_runtime_resref(pvr.baseclass_0.resref, actualPageResref) ||
      !game::matches_pvrz_page_identity(game::resref_view(actualPageResref),
                                        expectedTilesetResref, entry.page)) {
    return false;
  }

  out.texture = static_cast<unsigned>(pvr.texture);
  out.width = pvr.size.cx;
  out.height = pvr.size.cy;
  out.resref = actualPageResref;
  return true;
}

std::optional<game::PaletteTileAverage> read_pvrz_tint_page(
    const PvrzTintCandidate& candidate) {
  auto& gl = game::gl::get_gl_functions();
  if (!gl.glActiveTexture || !gl.glBindTexture || !gl.glGetIntegerv ||
      !gl.glGetTexLevelParameteriv || !gl.glGetTexImage || !gl.glPixelStorei ||
      !gl.glGetError) {
    return std::nullopt;
  }
  ...
  if (width != candidate.width || height != candidate.height || width <= 0 || height <= 0 ||
      width > kMaxTintPageDimension || height > kMaxTintPageDimension) {
    return std::nullopt;
  }
...
std::array<float, 3> resolve_snapshot_tint(const AreaGpuSnapshot& snapshot) {
  double tintSum[3] = {
      static_cast<double>(snapshot.waterTint[0]) * snapshot.paletteTintPixels,
      static_cast<double>(snapshot.waterTint[1]) * snapshot.paletteTintPixels,
      static_cast<double>(snapshot.waterTint[2]) * snapshot.paletteTintPixels,
  };
  std::size_t tintPixels = snapshot.paletteTintPixels;
  std::size_t pvrzPages = 0;
  for (const auto& candidate : snapshot.pvrzTintCandidates) {
    const auto average = read_pvrz_tint_page(candidate);
    if (!average) {
      LOG_WARN("PVRZ liquid tint readback failed for {} (texture {}, {}x{})",
               game::resref_view(candidate.resref), candidate.texture, candidate.width,
               candidate.height);
      continue;
    }
    ...
    tintPixels += average->opaquePixelCount;
    ++pvrzPages;
  }

  if (tintPixels == 0) {
    if (snapshot.fallbackTint) {
      const auto& fallback = *snapshot.fallbackTint;
      LOG_INFO("Area liquid tint fallback for {}: ({:.3f}, {:.3f}, {:.3f})",
               game::resref_view(fallback.tilesetResref), fallback.linearRgb[0],
               fallback.linearRgb[1], fallback.linearRgb[2]);
      return fallback.linearRgb;
    }
    if (!snapshot.pvrzTintCandidates.empty()) {
      LOG_WARN("Area liquid tint unavailable after {} PVRZ readback attempt(s); using neutral "
               "fallback",
               snapshot.pvrzTintCandidates.size());
    }
    return snapshot.waterTint;
  }

  const auto inverse = 1.0 / static_cast<double>(tintPixels);
  const std::array<float, 3> tint{
      static_cast<float>(tintSum[0] * inverse),
      static_cast<float>(tintSum[1] * inverse),
      static_cast<float>(tintSum[2] * inverse),
  };
  if (pvrzPages > 0) {
    LOG_INFO(
        "Area liquid tint: ({:.3f}, {:.3f}, {:.3f}) from {} visible pixels across {} palette "
        "tiles and {} PVRZ pages",
        tint[0], tint[1], tint[2], tintPixels, snapshot.paletteTintTiles, pvrzPages);
  }
  return tint;
}
...
std::optional<water_route2::Match> route2_water_overlay_match(
  ...
  if (!core::safe_read(areaBytes + offsetof(game::CGameArea, m_pResWED), liveWed) ||
      ...
      game::resref_view(wedName) != wed->areaResrefView()) return reject(3);
  ...
  if (!readResources(tileSets[0], baseResources, baseCount) ||
...
void refresh_wed_cache(AppContext& ctx, void* infGame) {
  ...
  try {
    ...
    for (std::size_t overlayIndex = 1;
         overlayIndex < cachedWed->overlays.size() && overlayIndex <= 7; ++overlayIndex) {
      const auto& overlay = cachedWed->overlays[overlayIndex];
      if (overlay.coverageCells == 0 || overlay.liquidMode == game::TileLiquidMode::None ||
          overlay.liquidMode == game::TileLiquidMode::Lava) {
        continue;
      }
      if (const auto tint =
              game::liquid_tileset_fallback_tint(overlay.tilesetResrefView())) {
        fallbackTint = AreaFallbackTint{.linearRgb = *tint,
                                        .tilesetResref = overlay.tilesetResref};
        break;
      }
    }
    std::array<game::CInfTileSet*, 5> tileSets{};
    ...
    const auto sampleTint = [&](std::size_t overlayIndex, std::uint16_t tileIndex) {
      ...
      if (!core::safe_read(tileResources + tileIndex, wrapper) || !wrapper ||
          !core::safe_read(wrapper, tileResource) || !tileResource.tis) {
        return false;
      }

      game::CRes tileRes{};
      if (!core::safe_read(tileResource.tis, tileRes) || !tileRes.bLoaded || !tileRes.pData) {
        return false;
      }

      if (tileRes.nSize >= game::kPaletteTileBytes) {
        // Work from one stable local snapshot. is_readable() proves page
        // access, not object lifetime, and repeatedly traversing mutable
        // engine memory while calculating a palette average would widen that
        // race window.
      ...
      PvrzTintCandidate candidate{};
      if (!read_pvrz_tint_candidate(wrapper, tileIndex,
                                    cachedWed->overlays[overlayIndex].tilesetResrefView(),
                                    candidate)) {
        return false;
      }
      for (const auto& existing : pvrzTintCandidates) {
        if (existing.texture == candidate.texture) return true;
      }
      pvrzTintCandidates.push_back(candidate);
      return true;
    };

    for (std::size_t overlayIndex = 1;
         overlayIndex < cachedWed->overlays.size() && overlayIndex <= 7 &&
         tintTiles + pvrzTintCandidates.size() < kMaxTintTileSamples &&
         tintAttempts < kMaxTintTileAttempts;
         ++overlayIndex) {
      const auto& overlay = cachedWed->overlays[overlayIndex];
      ...
      for (const auto tileIndex : overlay.tintTileCandidates) {
        ...
        if (tintTiles + pvrzTintCandidates.size() >= kMaxTintTileSamples ||
            tintAttempts >= kMaxTintTileAttempts) {
          break;
        }
      }
    ...
    const auto pvrzTintPageCount = pvrzTintCandidates.size();
    queue_area_gpu_snapshot(std::move(*packed), waterTint, tintOpaquePixels, tintTiles,
                            std::move(pvrzTintCandidates), std::move(fallbackTint));
    ...
    if (pvrzTintPageCount > 0) {
      LOG_INFO("Area liquid tint queued from {} PVRZ page(s)", pvrzTintPageCount);
    }
...
bool flush_pending_gpu_upload() noexcept {
  try {
    ...
    const auto waterTint = resolve_snapshot_tint(*snapshot);
    ...
    g_failedAreaTextureContext = nullptr;
    g_failedAreaGeneration = 0;
    probe::set_area_water_tint(waterTint[0], waterTint[1], waterTint[2]);
