...
namespace iee::game {
    ...
    std::optional<PaletteTileAverage> rgba_image_average_color(const std::uint8_t *data,
                                                               std::size_t width,
                                                               std::size_t height) {
        ...
        std::array<double, 256> linearChannel{};
        for (std::size_t encoded = 0; encoded < linearChannel.size(); ++encoded) {
            linearChannel[encoded] = srgb_to_linear(static_cast<std::uint8_t>(encoded));
        }
        ...
        for (std::size_t pixel = 0; pixel < pixelCount; ++pixel) {
            ...
            sumR += linearChannel[data[offset + 0]];
            sumG += linearChannel[data[offset + 1]];
            sumB += linearChannel[data[offset + 2]];
            ++opaqueCount;
        }
        if (opaqueCount == 0) {
            return std::nullopt;
