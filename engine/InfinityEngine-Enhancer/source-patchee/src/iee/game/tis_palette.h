...
namespace iee::game {
    ...
    // Average visible RGBA pixels from a GPU-decompressed PVRZ page. Fully
    // transparent atlas padding is ignored, matching the transparency rule of
    // classic liquid overlays. RGB is returned in linear light for the water
    // shader. Returns nullopt for malformed dimensions or an empty image.
    [[nodiscard]] std::optional<PaletteTileAverage> rgba_image_average_color(
