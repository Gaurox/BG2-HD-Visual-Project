...
std::atomic<unsigned> g_textureFilterDriftLogs{0};
std::atomic<bool> g_tileMipmapsUnavailableLogged{false};
...
bool configure_bound_texture(const core::EngineConfig& cfg, int sourceTextureId) {
  ...
  const bool forceTextureFilter = cfg.forceTextureFilterEveryDraw;
  const int expectedMinFilter = static_cast<int>(
      cfg.enableTileMipmaps ? gl::LINEAR_MIPMAP_LINEAR : gl::LINEAR);
  ...
  const bool logDetails = callCount <= 3 ||
                          (cfg.enableVerboseLogging && !forceTextureFilter);
  ...
  if (configured != configuredEnd) {
    ...
    if (!forceTextureFilter && configured->hitsUntilValidation > 1) {
    ...
    gl.glGetTexParameteriv(gl::TEXTURE_2D, gl::TEXTURE_WRAP_T, &wrapT);
    gl.glGetTexParameteriv(gl::TEXTURE_2D, gl::TEXTURE_MIN_FILTER, &minFilter);
    gl.glGetTexParameteriv(gl::TEXTURE_2D, gl::TEXTURE_MAG_FILTER, &magFilter);
    const bool parametersMatch =
        wrapS == static_cast<int>(gl::CLAMP_TO_EDGE) &&
        wrapT == static_cast<int>(gl::CLAMP_TO_EDGE) &&
        minFilter == expectedMinFilter && magFilter == static_cast<int>(gl::LINEAR);
    if (checkError("validating cached texture parameters") && parametersMatch &&
        !forceTextureFilter) {
      if (recordStats) {
        g_textureConfigurationCacheHits.fetch_add(1, std::memory_order_relaxed);
      }
    ...
    if (forceTextureFilter && !parametersMatch &&
        g_textureFilterDriftLogs.fetch_add(1, std::memory_order_relaxed) < 16) {
      LOG_WARN("Tile texture state drift before forced restore (source {}, GL {}, wrapS=0x{:X}, "
               "wrapT=0x{:X}, min=0x{:X}, mag=0x{:X})",
               sourceTextureId, boundTexture, wrapS, wrapT, minFilter, magFilter);
    }
  }
  ...
  if (cfg.enableTileMipmaps) {
    if (!gl.glGenerateMipmap) {
      if (!g_tileMipmapsUnavailableLogged.exchange(true, std::memory_order_relaxed)) {
        LOG_WARN("Tile mipmaps were requested but glGenerateMipmap is unavailable");
      }
      storeResult(true);
      return false;
    }
    if (!setInteger(gl::TEXTURE_MAX_LEVEL, 1000, "allowing generated texture mipmaps")) {
      storeResult(true);
      return false;
    }
    gl.glGenerateMipmap(gl::TEXTURE_2D);
    if (!checkError("generating tile texture mipmaps")) {
      storeResult(true);
      return false;
    }
  }

  if (!setInteger(gl::TEXTURE_MIN_FILTER, expectedMinFilter, "setting texture min filter")) {
    storeResult(true);
    return false;
  }
  ...
  if (logDetails) {
    LOG_DEBUG("Configured source texture {} (GL name {}, mipmaps={}, anisotropy={}, LOD bias={:.2f})",
              sourceTextureId, boundTexture,
              cfg.enableTileMipmaps,
