...
namespace iee::core {
    ...
    bool is_read_write_non_executable_section(const ModuleSpan &module,
                                               std::uintptr_t rva,
                                               std::size_t len) noexcept {
#ifdef _WIN64
        if (!module.base || len == 0 || rva > module.size || len > module.size - rva) {
            return false;
        }

        IMAGE_DOS_HEADER dos{};
        if (!safe_read(module.base, dos) || dos.e_magic != IMAGE_DOS_SIGNATURE ||
            dos.e_lfanew < 0) {
            return false;
        }
        const auto ntOffset = static_cast<std::size_t>(dos.e_lfanew);
        IMAGE_NT_HEADERS64 nt{};
        if (ntOffset > module.size || sizeof(nt) > module.size - ntOffset ||
            !safe_read(module.base + ntOffset, nt) || nt.Signature != IMAGE_NT_SIGNATURE) {
            return false;
        }

        const auto sectionTableOffset =
            ntOffset + offsetof(IMAGE_NT_HEADERS64, OptionalHeader) +
            nt.FileHeader.SizeOfOptionalHeader;
        for (std::size_t index = 0; index < nt.FileHeader.NumberOfSections; ++index) {
            const auto sectionOffset = sectionTableOffset + index * sizeof(IMAGE_SECTION_HEADER);
            IMAGE_SECTION_HEADER section{};
            if (sectionOffset > module.size || sizeof(section) > module.size - sectionOffset ||
                !safe_read(module.base + sectionOffset, section)) {
                return false;
            }

            const auto sectionRva = static_cast<std::size_t>(section.VirtualAddress);
            const auto sectionSize =
                std::max(static_cast<std::size_t>(section.Misc.VirtualSize),
                         static_cast<std::size_t>(section.SizeOfRawData));
            if (rva < sectionRva) continue;
            const auto offsetInSection = static_cast<std::size_t>(rva - sectionRva);
            if (offsetInSection > sectionSize || len > sectionSize - offsetInSection) continue;
    ...
    bool is_writable_non_executable_memory(const void *p, std::size_t len) noexcept {
#ifdef _WIN64
        if (!p || len == 0) return false;
        auto current = reinterpret_cast<std::uintptr_t>(p);
        if (len > std::numeric_limits<std::uintptr_t>::max() - current) return false;
        const auto end = current + len;
        ...
        while (current < end) {
            ...
            const auto regionBase = reinterpret_cast<std::uintptr_t>(mbi.BaseAddress);
            if (mbi.RegionSize > std::numeric_limits<std::uintptr_t>::max() - regionBase) {
                return false;
            }
