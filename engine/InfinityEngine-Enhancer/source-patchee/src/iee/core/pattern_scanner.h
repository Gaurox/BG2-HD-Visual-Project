...
namespace iee::core {
    ...
    // Validates immutable PE-section characteristics for a complete module RVA
    // range. Used for engine scratch/global data that must never resolve into
    // executable code.
    [[nodiscard]] bool is_read_write_non_executable_section(
        const ModuleSpan &module, std::uintptr_t rva, std::size_t len) noexcept;

    // Validates the current page protections for a complete address range.
    // Only committed RW/WC pages are accepted; executable or guarded pages fail.
    [[nodiscard]] bool is_writable_non_executable_memory(const void *p,
