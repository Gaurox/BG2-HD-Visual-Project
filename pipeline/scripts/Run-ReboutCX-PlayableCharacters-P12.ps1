param(
    ...
    [ValidateRange(512, 32768)] [int]$MemoryMiB = 4096,
    [ValidateRange(0, 32768)] [int]$CacheMiB = 1024
)
...
$runner = Join-Path $PSScriptRoot 'reboutcx_playable_p12.py'
if ($Run) {
    & $python -B $runner run $Queue --components $Components --pre-workers $PreWorkers --post-workers $PostWorkers --memory-mib $MemoryMiB --cache-mib $CacheMiB
} else {
    & $python -B $runner plan $Queue
}
if ($LASTEXITCODE -ne 0) { throw 'P12 command failed' }
