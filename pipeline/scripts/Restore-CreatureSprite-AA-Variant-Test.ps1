...
function Assert-GameChild([string]$GameRoot, [string]$RelativePath) {
    if ([IO.Path]::IsPathRooted($RelativePath)) { throw "Cible AA absolue interdite : $RelativePath" }
...
if (-not (Test-Path -LiteralPath $statePath -PathType Leaf)) { throw 'Aucun état AA actif à restaurer.' }
if (@(Get-Process -Name 'InfinityLoader', 'Baldur', 'BaldurReal' -ErrorAction SilentlyContinue).Count -ne 0) {
    throw 'Le jeu ou InfinityLoader est actif. Ferme-le avant la restauration AA.'
}
$state = Get-Content -LiteralPath $statePath -Raw | ConvertFrom-Json
if ($state.schema -ne 'bg2-upscale-creature-sprite-aa-variant-test-v1' -or
    $state.job_id -ne $job.job_id -or
    -not [string]::Equals([string]$state.game_root, $gameRoot, [StringComparison]::OrdinalIgnoreCase)) {
    throw 'État AA incompatible avec le job ou le jeu.'
}
$interrupted = $state.status -in @('installing', 'restoring')
if ($interrupted -and -not $RecoverInterrupted) {
    throw "Transaction AA interrompue ($($state.status)); relance avec -RecoverInterrupted."
}
if (-not $interrupted -and $state.status -notin @('installed-pending-qa', 'qa-failed')) {
    throw "État AA non restaurable : $($state.status)"
}
...
if (-not ($backupRoot + '\').StartsWith($backupBase, [StringComparison]::OrdinalIgnoreCase)) {
    throw "backup_root AA hors du job : $backupRoot"
}
$backupStatePath = Join-Path $backupRoot 'install-state.json'
if (-not (Test-Path -LiteralPath $backupStatePath -PathType Leaf)) { throw 'État de sauvegarde AA absent.' }
$targets = @($state.targets)
if ($targets.Count -lt 5 -or $targets.Count -gt 69) { throw 'Liste de cibles AA hors contrat.' }
...
foreach ($targetState in $targets) {
    ...
    if (-not $allowed -or -not $seen.Add($relative)) { throw "Cible AA invalide : $relative" }
    ...
    if (-not $interrupted) {
        ...
        if ([bool]$targetState.installed_present -ne $present) {
            throw "Présence modifiée depuis le test AA : $relative"
        }
        if ($present -and (Get-Sha256 $target) -ne [string]$targetState.installed_sha256) {
            throw "Cible modifiée depuis le test AA : $relative"
        }
    }
    if ([bool]$targetState.existed_before) {
        ...
        if (-not [string]::Equals($backup, $expected, [StringComparison]::OrdinalIgnoreCase) -or
            ...
            throw "Sauvegarde AA absente ou altérée : $relative"
        }
    }
}
...
foreach ($targetState in $targets) {
    ...
    if ([bool]$targetState.existed_before) {
        if (-not (Test-Path -LiteralPath $target -PathType Leaf) -or
            ...
            throw "Restauration AA non fidèle : $($targetState.relative_path)"
        }
    }
    elseif (Test-Path -LiteralPath $target -PathType Leaf) {
        throw "Fichier AA ajouté encore présent : $($targetState.relative_path)"
    }
}
