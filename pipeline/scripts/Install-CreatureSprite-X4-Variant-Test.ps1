...
if (-not $jobFull.StartsWith($jobsRoot, [StringComparison]::OrdinalIgnoreCase)) {
    throw "Le job x4 doit rester sous sprite/jobs : $jobFull"
}
...
if ($job.schema -ne 'bg2-upscale-creature-sprite-xbr2x-job-v1' -or
    ...
    [string]$job.animation.id -ne '0x6102' -or [string]$job.animation.bam_prefix -ne 'CDMB1' -or
    [int]$job.animation.armor_code -ne 1 -or
    [int]$job.upscale.scale -ne 4 -or [string]$job.upscale.algorithm -ne 'XBR/xbr4X' -or
    [int]$job.upscale.passes -ne 1 -or [bool]$job.upscale.antialias -or [bool]$job.upscale.xbr_blend) {
...
$expectedRunRoot = [IO.Path]::GetFullPath((Join-Path $projectRoot 'sprite\dwarf-male-fighter-cdmb1-x4\runs\xbr4x-x4'))
if (-not [string]::Equals($runRoot, $expectedRunRoot, [StringComparison]::OrdinalIgnoreCase)) {
    throw "run_dir x4 inattendu : $runRoot"
}
...
foreach ($required in @($buildManifestPath, $runtimeManifestPath, $registrySource, $dllSource)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) { throw "Artefact x4 absent : $required" }
}
...
if ($build.schema -ne 'bg2-upscale-creature-sprite-xbr2x-pack-v1' -or
    $build.status -ne 'built-pending-ingame-qa' -or $build.job_id -ne $job.job_id -or
    [int]$build.registry_version -ne 3 -or [int]$build.registry_scale -ne 4 -or
    $build.registry_layout -ne 'monolith' -or
    [string]$build.registry -ne 'iee-assets/creature-sprites/CreatureSprites-XN.registry' -or
    [int]$build.resource_count -ne 23 -or [int]$build.frame_count -ne 10323 -or
    [string]$build.method.algorithm -ne 'XBR/xbr4X' -or [int]$build.method.scale -ne 4 -or
    [int]$build.method.passes -ne 1 -or [bool]$build.method.antialias -or [bool]$build.method.xbr_blend -or
    [string]$build.registry_sha256 -ne (Get-Sha256 $registrySource)) {
    throw 'Build x4 non vérifiable.'
}
if ($runtime.schema -ne 'bg2-upscale-creature-sprite-runtime-v1' -or
    ...
    $runtime.job_id -ne $job.job_id -or $runtime.dll_sha256 -ne (Get-Sha256 $dllSource)) {
    throw 'Runtime x4 non vérifiable.'
}
$exe = Join-Path $gameRoot 'BaldurReal.exe'
if ((Get-Sha256 $exe) -ne ([string]$job.compatibility.baldur_real_sha256).ToUpperInvariant()) {
    throw 'BaldurReal.exe diffère du job x4.'
}
if (@(Get-Process -Name 'InfinityLoader', 'Baldur', 'BaldurReal' -ErrorAction SilentlyContinue).Count -ne 0) {
    throw "Le jeu ou InfinityLoader est actif. Ferme-le avant l'installation x4."
}

$overridePath = Join-Path $gameRoot 'override'
if (Test-Path -LiteralPath $overridePath -PathType Container) {
    $collisions = @(Get-ChildItem -LiteralPath $overridePath -Filter 'CDMB1*.BAM' -File -ErrorAction SilentlyContinue |
        Where-Object { -not [string]::Equals($_.Name, 'CDMB1INV.BAM', [StringComparison]::OrdinalIgnoreCase) })
    if ($collisions.Count -ne 0) { throw "Collision override CDMB1 : $($collisions.Name -join ', ')" }
}
...
if (Test-Path -LiteralPath $activeStatePath -PathType Leaf) {
    ...
    if ($existing.status -in @('installing', 'restoring', 'installed-pending-qa', 'qa-failed')) {
        throw "La variante x4 est déjà active ou interrompue : $($existing.status)"
    }
}
...
foreach ($candidate in Get-ChildItem -LiteralPath $spriteRoot -Filter 'active-test.json' -File -Recurse -ErrorAction SilentlyContinue) {
    ...
    if ($candidateState.status -in @('installing', 'restoring', 'installed-pending-qa', 'validated-installed', 'qa-failed')) {
        if (-not [string]::Equals([string]$candidateState.game_root, $gameRoot, [StringComparison]::OrdinalIgnoreCase)) {
            throw "Test sprite actif sur un autre GameRoot : $($candidate.FullName)"
        }
...
if ($VerifyOnly) {
    [pscustomobject]@{
        Status = 'install-preflight-verified'
        Job = [string]$job.job_id
        ParentActiveTests = $parentStates.Count
        RegistrySha256 = Get-Sha256 $registrySource
        DllSha256 = Get-Sha256 $dllSource
...
if ($relativeTargets.Count -gt 69) { throw 'Namespace xN actif hors limites.' }
...
foreach ($relative in $relativeTargets) {
    ...
    if ($existed) {
        ...
        if ((Get-Sha256 $backup) -ne $hash) { throw "Sauvegarde x4 non fidèle : $relative" }
    }
    $targets += [ordered]@{
...
$state = [ordered]@{
    schema = 'bg2-upscale-creature-sprite-x4-variant-test-v1'
    status = 'installing'
...
try {
    ...
    $dllTarget = Assert-GameChild $gameRoot 'InfinityEngine-Enhancer.dll'
    ...
    if (-not (Test-Path -LiteralPath $iniTarget -PathType Leaf)) {
        throw 'InfinityEngine-Enhancer.ini doit exister avant le test x4.'
    }
    ...
    if ((Get-Sha256 $registryTarget) -ne (Get-Sha256 $registrySource)) { throw 'Registre x4 installé non fidèle.' }
    if ((Get-Sha256 $dllTarget) -ne (Get-Sha256 $dllSource)) { throw 'DLL x4 installée non fidèle.' }
