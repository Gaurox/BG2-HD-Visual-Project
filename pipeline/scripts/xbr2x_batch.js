...
// Binary batch adapter around the exact xBR2X implementation embedded in the
// local MMPX/scalepix page. The page is initialized once for the whole sprite.

const fs = require('fs');
const vm = require('vm');

const scalepixPath = process.argv[2];
if (!scalepixPath) throw new Error('Usage: node xbr2x_batch.js <scalepix.html>');
...
if (input.length < 12 || !input.subarray(0, 8).equals(INPUT_MAGIC)) {
  throw new Error('invalid xBR2 batch input');
}
...
if (offset !== input.length) throw new Error('trailing bytes in xBR2 batch input');
