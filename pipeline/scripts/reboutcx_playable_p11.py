"""Explicit P11 queues, longest jobs first, one GPU owner, asynchronous verification."""
...
from reboutcx_full_p11 import (RUN_ID, derive_job, execute, read_json, relative,
                             resolve_path_reference, sha256_file, validate_job, verify)
from reboutcx_runtime_p11 import Runtime

QUEUE_SCHEMA = "bg2-upscale-reboutcx-p11-queue-v1"
...
def prepare_queue(family_jobs: list[Path], destination: Path) -> dict:
    ...
    for path in family_jobs:
        ...
        # Never bootstrap, audit or prepare P8 in the P11 controller.
        for member in read_json(prepared)["members"]:
            ...
            if read_json(source).get("runtime_profile") != "character-bg2ee-2.7.3.0":
                raise RuntimeError("P11 playable queue requires Character component jobs")
...
def load_queue(path: Path) -> list[Path]:
    ...
    if queue.get("schema") != QUEUE_SCHEMA or queue.get("installable") is not False:
        raise RuntimeError("invalid P11 queue")
    ...
    for member in queue["members"]:
        ...
        if sha256_file(job_path) != member["sha256"]:
            raise RuntimeError("P11 queue job hash differs")
        job = read_json(job_path)
        validate_job(job)
        if job.get("runtime_profile") != "character-bg2ee-2.7.3.0":
            raise RuntimeError("P11 queue is restricted to Character jobs")
        ...
        if model_contract is not None and model_contract != model:
            raise RuntimeError("P11 queue contains multiple model/device/precision contracts")
        model_contract = model
        output = resolve_path_reference(job["paths"]["run_dir"]).resolve()
        if output in outputs:
            raise RuntimeError("P11 queue contains duplicate output directories")
        outputs.add(output)
...
def priority(path: Path) -> float:
    job = read_json(path)
    source_job = resolve_path_reference(job["p11_source_job"]["path"], required=True)
    if sha256_file(source_job) != job["p11_source_job"]["sha256"]:
        raise RuntimeError("source job changed after P11 derivation")
...
def run_queue(path: Path, *, components: int = 3, pre_workers: int = 2,
              post_workers: int = 3, memory_mib: int = 4096, log_dir: Path | None = None) -> dict:
    jobs = load_queue(path)
    if not jobs:
        raise RuntimeError("P11 queue is empty")
    ...
    logs = log_dir if log_dir is not None else Path(__file__).resolve().parents[2] / "sprite/.work/reboutcx-p11-production"
