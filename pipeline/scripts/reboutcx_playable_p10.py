"""Explicit P10 queues, longest jobs first, one GPU owner, asynchronous verification."""
...
from reboutcx_full_p10 import (RUN_ID, derive_job, execute, read_json, relative,
                             resolve_path_reference, sha256_file, validate_job, verify)
from reboutcx_runtime_p10 import Runtime

QUEUE_SCHEMA = "bg2-upscale-reboutcx-p10-queue-v1"
...
def prepare_queue(family_jobs: list[Path], destination: Path) -> dict:
    ...
    for path in family_jobs:
        ...
        # Never bootstrap, audit or prepare P8 in the P10 controller.
        for member in read_json(prepared)["members"]:
            ...
            if read_json(source).get("runtime_profile") != "character-bg2ee-2.7.3.0":
                raise RuntimeError("P10 playable queue requires Character component jobs")
...
def load_queue(path: Path) -> list[Path]:
    ...
    if queue.get("schema") != QUEUE_SCHEMA or queue.get("installable") is not False:
        raise RuntimeError("invalid P10 queue")
    ...
    for member in queue["members"]:
        ...
        if sha256_file(job_path) != member["sha256"]:
            raise RuntimeError("P10 queue job hash differs")
        job = read_json(job_path)
        validate_job(job)
        if job.get("runtime_profile") != "character-bg2ee-2.7.3.0":
            raise RuntimeError("P10 queue is restricted to Character jobs")
        ...
        if model_contract is not None and model_contract != model:
            raise RuntimeError("P10 queue contains multiple model/device/precision contracts")
        model_contract = model
        output = resolve_path_reference(job["paths"]["run_dir"]).resolve()
        if output in outputs:
            raise RuntimeError("P10 queue contains duplicate output directories")
        outputs.add(output)
...
def priority(path: Path) -> float:
    job = read_json(path)
    source_job = resolve_path_reference(job["p10_source_job"]["path"], required=True)
    if sha256_file(source_job) != job["p10_source_job"]["sha256"]:
        raise RuntimeError("source job changed after P10 derivation")
...
def run_queue(path: Path, *, components: int = 3, pre_workers: int = 2,
              post_workers: int = 3, memory_mib: int = 2048, log_dir: Path | None = None) -> dict:
    jobs = load_queue(path)
    if not jobs:
        raise RuntimeError("P10 queue is empty")
    ...
    logs = log_dir if log_dir is not None else Path(__file__).resolve().parents[2] / "sprite/.work/reboutcx-p10-production"
    ...
    with log_path.open("x", encoding="utf-8") as log, Runtime(
            pre_workers=pre_workers, post_workers=post_workers, memory_mib=memory_mib) as runtime, \
        ...
        report = {"completed": completed, "new_model_frames": total_frames,
                  ...
                  "new_model_frames_per_second": total_frames / elapsed,
...
def main() -> int:
    ...
    for name in ("plan", "run"):
        ...
        if name == "run":
            ...
            cmd.add_argument("--memory-mib", type=int, default=2048)
    ...
    if args.command == "prepare":
        ...
        run_queue(args.queue, components=args.components, pre_workers=args.pre_workers,
                  post_workers=args.post_workers, memory_mib=args.memory_mib)
    return 0
