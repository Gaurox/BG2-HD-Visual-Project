...
def build(temporal_run: Path, resref: str, mask_path: Path, output: Path,
    ...
    require(temporal_manifest.get("timed_resources") == [resref],
            "le run temporel source doit couvrir exactement le resref masqué")
    source_pack = temporal_run / "03_runtime_pack"
    source_pack_manifest, source_resources = temporal.validate_v2_pack(source_pack)
    source_resource = next((copy.deepcopy(item) for item in source_resources
                            if item["resref"] == resref), None)
    require(source_resource is not None and source_resource["playback_mode"] == "TimedTimeline",
            f"{resref}: ressource temporelle absente")
    ...
    base_resource = next((item for item in base_resources if item["resref"] == resref), None)
    require(base_resource is not None and base_resource["playback_mode"] == "Native",
            f"{resref}: les anchors de base doivent être Native")
    anchor_count = int(base_resource["frame_count"])
    require(anchor_count < int(source_resource["frame_count"]),
            f"{resref}: aucune phase temporelle à masquer")
    require(all(source_resource["frames"][index]["asset"] == base_resource["frames"][index]["asset"] and
            ...
            f"{resref}: anchors du run temporel divergentes du pack de base")
    ...
    require(all(tuple(int(value) for value in frame["physical_size_x4"]) == dimensions
            ...
            f"{resref}: masque répété exige une géométrie uniforme")
    mask = load_mask(mask_path.resolve(), dimensions)
    ...
    manual_masks.mkdir()
    for index in range(anchor_count):
        shutil.copyfile(mask_path, manual_masks / f"frame_{index:03d}.png")
    ...
    anchor_names = {str(frame["asset"]) for frame in base_resource["frames"]}
    ...
    for frame in resource["frames"]:
        ...
        payload = masked_rgba(asset_path, frame["physical_size_x4"], mask)
    ...
    pack_manifest["manual_alpha_patch"] = {
        "schema": MASK_SCHEMA,
        "resref": resref,
        "mask_source": mask_path.resolve().as_posix(),
        "mask_sha256": sha256_file(mask_path),
        "mask_size_x4": list(dimensions),
        "mask_assignment": "single mask replicated to every anchor and interpolation phase",
    ...
    work_root = partial / "work" / resref / "cycle_000"
    reviews = render_reviews(pack_root, resource, work_root, review_ffmpeg)
    mask_record = {
        "schema": MASK_SCHEMA,
        "status": "completed",
        "resref": resref,
        ...
        "masked_frame_count": int(resource["frame_count"]),
        "alpha_formula": "alpha_final = alpha_source * grayscale_mask / 255",
        "rgb_policy": "byte-identical",
    }
