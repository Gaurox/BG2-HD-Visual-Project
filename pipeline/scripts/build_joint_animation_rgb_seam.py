...
SCHEMA = "bg2-upscale-animation-joint-rgb-seam-batch-v2"
...
def update_resource(resource: dict[str, Any], frames: list[dict[str, Any]],
                    pixels: list[np.ndarray]) -> dict[str, Any]:
...
def write_runtime_pack(destination: Path, source: PackInput, frames: list[dict[str, Any]],
                       pixels: list[np.ndarray], provenance: dict[str, Any]) -> dict[str, Any]:
    resource = update_resource(source.resource, frames, pixels)
...
def inspect(top: PackInput, bottom: PackInput, seam_depth_x4: int,
            ...
            restore_removed_component_min_pixels: int) -> dict[str, Any]:
...
def build(top: PackInput, bottom: PackInput, output: Path, *, seam_depth_x4: int,
          ...
          restore_removed_component_min_pixels: int = 0) -> dict[str, Any]:
    ...
    for index, (top_frame, bottom_frame) in enumerate(zip(top_frames, bottom_frames, strict=True)):
        ...
        reports.append({"frame": index, "alpha_restore": alpha_restore_report, **report})
        ...
        previews_after.append(compose_preview(
            repaired_top, repaired_bottom, top_layout=top_layout, bottom_layout=bottom_layout,
            seam_depth_x4=seam_depth_x4,
        ))
    ...
    top_manifest = write_runtime_pack(partial / top_pack, top, top_frames, output_top, repair_provenance)
    bottom_manifest = write_runtime_pack(partial / bottom_pack, bottom, bottom_frames, output_bottom, repair_provenance)
