"""Versioned P10 full component runner. No writes to P8/P9 or derived catalogs."""
...
from reboutcx_runtime_p10 import Runtime, ResourceWindow, postprocess_batch

RUN_ID = "reboutcx-p10-stream86-v1"
BATCH_CONTRACT = {
    "id": "p10-normalized-padded86-v1", "max_frames": 86, "canvas_quantum": 64,
    "input": "uint8-to-float32-div255-before-fp16",
    "padding": "zero-top-left-before-model; crop-to-source-after-model",
    "grouping": "canonical-resource-order; geometry-buckets; no-duplicates",
...
def validate_job(job: dict) -> None:
    if (job.get("schema") != JOB_SCHEMA or job.get("installable") is not False
        ...
        raise RuntimeError("P10 requires its own versioned, non-installable job/run contract")
...
def derive_job(source_path: Path, destination: Path | None = None) -> Path:
    ...
    result["p10_source_job"] = {"path": relative(source), "sha256": sha256_file(source)}
    validate_job(result)
    output = destination.resolve() if destination else source.with_name(RUN_ID + ".json")
    if output.exists():
        if read_json(output) != result:
            raise RuntimeError("existing P10 job differs; select a new version")
    else:
...
def render_contract_digest(job: dict, classes: dict, palette_evidence: dict | None) -> str:
    value = {"base": p8.render_contract_digest(job, classes, palette_evidence),
             "p10": job["reboutcx_batch"]}
    return hashlib.sha256(json.dumps(value, sort_keys=True).encode()).hexdigest().upper()


def render_resource(*, resource: dict, component_path: Path, animation_id: int,
                    runtime: Runtime, fp16: bool, states: dict, timing: dict,
                    qa_indices: set[int], classes: dict, reference_palette: np.ndarray | None):
    ...
    timing = defaultdict(float, timing)
    buckets = defaultdict(list)
    bypass = []
    for frame in frames:
        state = states[frame.index]
        if frame.index in qa_indices:
            state["xbr_rgba"] = p8.reconstruct_rgba(state["guide"], state["palette"], frame.transparent)
        ...
        if state["rgb"] is None:
            ...
            buckets[((height + 63) // 64 * 64, (width + 63) // 64 * 64)].append(state)
    ...
    def collect():
        ...
        for key, value in elapsed.items():
            timing[key] += value
        for index, quantized, metrics, target in results:
            states[index].update(quantized=quantized, metrics=metrics, target_rgb=target)

    def submit(items, crops):
        requests = [(s["frame"].index, s["guide"], s["palette"], np.unique(s["frame"].indices),
                     s["frame"].transparent, crop, s["frame"].index in qa_indices)
                    for s, crop in zip(items, crops, strict=True)]
        pending.append(runtime.post.submit(postprocess_batch, requests, classes))
        # Bound retained x4 crops/IPC; two post batches at most per component.
        if len(pending) >= 2:
            collect()

    try:
        for start in range(0, len(bypass), 86):
            items = bypass[start:start + 86]
            submit(items, [None] * len(items))
        for canvas, items in buckets.items():
            for start in range(0, len(items), 86):
                batch = items[start:start + 86]
                crops, elapsed = runtime.infer([s["rgb"] for s in batch], canvas, fp16).result()
                for key, value in elapsed.items():
                    timing[key] += value
        ...
        for future in pending:
            if not future.cancel():
                # Finish submitted work before its memory reservation can be released.
                try:
                    future.result()
                except Exception:
                    pass
        ...
        for frame in frames:
            ...
            if frame.index in qa_indices:
                ...
                if target is None or state["xbr_rgba"] is None:
                    raise RuntimeError(f"{resref} frame {frame.index}: missing P10 comparison pixels")
...
def _execute_payload(job_path: Path, runtime: Runtime, windows: list) -> dict[str, Any]:
    ...
    window = ResourceWindow(runtime, scalepix=scalepix, node=str(job["tools"].get("node", "node")),
    ...
    for resref in actual_inventory:
        ...
        if canonical == resref:
            states, preparation_timing = window.get(resource)
            position = canonical_order.index(resref)
            if position + 1 < len(canonical_order):
                window.submit(resource_by_resref[canonical_order[position + 1]], blocking=False)
    ...
    # Pin every imported historical helper as well as every P10 stage.
    for filename in ("reboutcx_full.py", "reboutcx_batch_p10.py", "reboutcx_cpu_p10.py",
                     "reboutcx_runtime_p10.py", "reboutcx_playable_p10.py", "workspace_paths.py"):
...
def verify(job_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("method", {}).get("batch") != BATCH_CONTRACT:
        raise RuntimeError("P10 manifest batch contract differs")
    if (
    ...
    if ([report["resref"] for report in manifest["resources"]] != expected_inventory
        ...
        raise RuntimeError("P10 resource inventory differs")
    checked = 0
    for resource in resources:
        resref = str(resource["source"]["name"]).upper()
