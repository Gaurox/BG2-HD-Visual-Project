"""Initialize the icon processing authority from the generated BAM inventory.

Existing rows are preserved byte-for-field. Newly inventoried assets are appended
with conservative lifecycle states. Use ``--run`` to write.
"""
...
PROCESSING = ROOT / "icons/index/processing.csv"
FIELDS = (
    "asset_key",
    "asset_id",
    "asset_directory",
...
def read_resources() -> list[dict[str, str]]:
    with RESOURCES.open(encoding="utf-8-sig", newline="") as stream:
        rows = list(csv.DictReader(stream))
    expected = {"asset_key", "resref", "source_sha256", "extracted_path"}
    if not rows or not expected.issubset(rows[0]):
        raise ValueError("resources.csv absent ou incomplet; régénérer l'inventaire icons")
    keys = [row["asset_key"] for row in rows]
    if len(keys) != len(set(keys)):
        raise ValueError("asset_key dupliqué dans icons/index/resources.csv")
    return rows


def icon_asset_id(resref: str) -> str:
    return f"icons:icon-{resref.lower()}"
...
def default_row(asset: Mapping[str, str]) -> dict[str, str]:
    ...
    return {
        ...
        "asset_id": icon_asset_id(resref),
        "asset_directory": f"icons/ressources/{resref}",
        "upscale_run": "",
...
def build_rows() -> tuple[list[dict[str, str]], int]:
    ...
    if len(existing) != len(existing_rows):
        raise ValueError("asset_key dupliqué dans icons/index/processing.csv")

    asset_keys = {row["asset_key"] for row in assets}
    stale = sorted(set(existing) - asset_keys, key=str.casefold)
    if stale:
        raise ValueError(
            "processing contient des assets absents de resources.csv: " + ", ".join(stale)
        )
    ...
    for asset in assets:
        key = asset["asset_key"]
        resref = asset["resref"].upper()
        expected_id = icon_asset_id(resref)
        ...
        if current is None:
            ...
            additions += 1
            continue
        if current["asset_id"] != expected_id:
            raise ValueError(f"asset_id invalide pour {key}: {current['asset_id']!r}")
...
def main() -> int:
    ...
    parser.add_argument("--run", action="store_true", help="écrit icons/index/processing.csv")
