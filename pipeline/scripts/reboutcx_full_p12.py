"""Versioned P12 full component runner. Earlier runs and derived catalogs are untouched."""
...
from reboutcx_runtime_p12 import (Runtime, GroupWindow, plan_groups, process_group,
    CANVAS_QUANTUM, GROUP_MAX_RESOURCES, GROUP_NATIVE_PIXEL_CAP)

from reboutcx_cache_p12 import context_key, PIXEL_CONTRACT

RUN_ID = "reboutcx-p12-cache86-v1"
BATCH_CONTRACT = {
    "id": "p12-cached-fixed86-q32-v1", "max_frames": 86, "canvas_quantum": CANVAS_QUANTUM,
    ...
    "pixel_cache": PIXEL_CONTRACT, "fixed_gpu_batch": 86,
...
def validate_job(job: dict) -> None:
    if (job.get("schema") != JOB_SCHEMA or job.get("installable") is not False
        ...
        raise RuntimeError("P12 requires its own versioned, non-installable job/run contract")


def derive_job(source_path: Path, destination: Path | None = None) -> Path:
    source = source_path.resolve()
    job = read_json(source)
    suffix = next((s for s in ("reboutcx-p11-q32-group86-v1", "reboutcx-p10-stream86-v1", "reboutcx-p9-batch86-v1", "reboutcx-p8-full-v1")
    ...
    if (suffix is None or job.get("schema") != JOB_SCHEMA or job.get("installable") is not False
        ...
        raise RuntimeError("derive-job requires a prepared P8/P9/P10/P11 component job")
    ...
    result["p12_source_job"] = {"path": relative(source), "sha256": sha256_file(source)}
    validate_job(result)
    output = destination.resolve() if destination else source.with_name(RUN_ID + ".json")
    if output.exists():
        if read_json(output) != result:
            raise RuntimeError("existing P12 job differs; select a new version")
    else:
        ...
        for frame in frames:
            ...
            frame_manifest.append(
                {
                    ...
                    "cache_reused": bool(state.get("cache_reused", False)),
                }
            )
            if frame.index in qa_indices:
                ...
                if target is None or state["xbr_rgba"] is None:
                    raise RuntimeError(f"{resref} frame {frame.index}: missing P12 comparison pixels")
...
def _execute_payload(job_path: Path, runtime: Runtime, windows: list) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "coverage": {
            ...
            "logical_model_frames": int(timings["logical_model_frames"]),
            "model_cache_hit_frames": int(timings["logical_model_frames"]-timings["model_frames"]),
            "gpu_slots": int(timings["gpu_slots"]),
            "gpu_filler_frames": int(timings["gpu_filler_frames"]),
        },
    ...
    # Pin every imported historical helper as well as every P12 stage.
    for filename in ("reboutcx_full.py", "reboutcx_batch_p10.py", "reboutcx_cpu_p10.py",
                     "reboutcx_runtime_p10.py", "reboutcx_runtime_p11.py", "reboutcx_runtime_p12.py",
                     "reboutcx_cache_p12.py", "reboutcx_batch_p12.py", "reboutcx_plan_p12.py",
                     "reboutcx_playable_p12.py", "workspace_paths.py"):
...
def verify(job_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("method", {}).get("batch") != BATCH_CONTRACT:
        raise RuntimeError("P12 manifest batch contract differs")
    if (
    ...
    if ([report["resref"] for report in manifest["resources"]] != expected_inventory
        ...
        raise RuntimeError("P12 resource inventory differs")
    canonical_records = [f for r in manifest["resources"] if not r["reused_render_from"] for f in r["frame_records"]]
    logical = sum(not f["model_bypassed"] for f in canonical_records)
    cached = sum(not f["model_bypassed"] and f["cache_reused"] for f in canonical_records)
    coverage = manifest["coverage"]
    if (coverage["logical_model_frames"] != logical or coverage["model_cache_hit_frames"] != cached
            or coverage["unique_model_frames"] != logical-cached
            or coverage["gpu_slots"] != 86*manifest["timing_seconds"].get("batches",0)
            or coverage["gpu_slots"] != coverage["unique_model_frames"]+coverage["gpu_filler_frames"]):
        raise RuntimeError("P12 logical/cache/actual/filler frame counts differ")
    checked = 0
    for resource in resources:
        resref = str(resource["source"]["name"]).upper()
