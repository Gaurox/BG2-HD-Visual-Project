...
| Occurrence en palette externe (`palette_mode=external`, ex. `AM3000E`/`PLT_MAGE`, `FLM2PUR` sur `OH4210`/`PLT_PURP`) | hors pipeline x4 sur cette occurrence précise : `split_animation_pack_by_area.py` l'exclut automatiquement, le resref reste `non-traité` tant qu'une zone en palette externe fait partie de son inventaire ; ne jamais installer le pack d'une autre zone (palette embarquée) dessus, la couleur serait fausse | le moteur consomme un jour la palette réalisée sur le chemin x4 |
...
| Contour épais, sombre ou crénelé d'un objet rigide non `Blended` après x4, surtout de jour (`AM5000A/B`, 2026-09-23) | cause RGB : liseré sombre de 1 px dans la source x1, étalé par SeedVR. Recette **`CONTOUR-NET`** (`pipeline/scripts/contour_net.py`) : Potrace x1 alphamax 0.8 + liseré remplacé par moyenne gaussienne de l'intérieur ; validée « parfaite » en jeu. Splines (surépaisseur, sommet arrondi), xBR (ondulation) et repoussage au plus proche (stries) écartés | ressource `Blended`, alpha source non nearest, ou contour organique où la spline suffit |
| Contour dentelé toujours visible après `derive_animation_visual_treatment.py --inner-contour-fade-x4` (`AM6200H/I/J`, signalé par l'utilisateur en jeu le 2026-09-22) | un fondu radial simple adoucit l'INTENSITÉ du bord, pas sa FORME : passer à la spline Fit 1 + feather 8 (recette portail), même sur une silhouette non `Blended` sans lien avec un portail | silhouette déjà lissée par une spline en amont |
| Balayage de rayon Blended qui ne discrimine pas (cœur p99 quasi inchangé à tous les rayons testés, ex. `AM4500*`/`AM6200A/B/E/F/G`) | prendre le plus grand rayon testé (bord le plus propre), pas le plus petit par défaut | balayage étendu qui montre une perte de cœur avant le plafond |
