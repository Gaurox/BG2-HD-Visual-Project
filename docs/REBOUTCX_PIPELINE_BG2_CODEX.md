...
- Orchestration bornée : `pipeline/scripts/reboutcx_character_family.py`, job `sprite/families/playable-characters/6100-human-male-fighter/family-runs/complete-reboutcx-p8-v1/jobs/human-male-fighter-complete-reboutcx-p8-v1.json`. Un composant par processus ; `--workers` règle la concurrence sans partager d'état ni de sortie.
- Audit CPU `audit.json`, SHA `276F1B3803B5E6F9EFFDE2935F3F231AF6616D26C20C2F283D2129113D061E00` : 65 composants = 4 body + 18 helmet + 12 shield + 31 weapon ; 656 ressources/180 337 frames, 28 850 marqueurs exacts. Estimation initiale 142 540 frames non marqueur ; production réelle 141 128 inférences, les 1 412 frames entièrement transparentes ne sollicitant pas le modèle. Transparence 0 et couverture des 256 indices par le contrat Character vérifiées ; 12 palettes BAM sources restent guides xBR, palette réalisée RANGES12 fixe pour ReboutCX.
- Les 65 composants xBR sont partagés avec d'autres animations : remplacer seulement `(0x6100, old_component_index)`. Ne jamais zipper `bam_prefixes` avec les indices triés ; mapper par ensemble RESREF exact. Contrôle final : 65 ensembles distincts, CHMB1→151, WQLS0→66, 65/65 partagés.
- Stabilité Windows : les trois arbres `.BG2HD-Installer-Windows*.tmp/` (40 725 fichiers non suivis) déclenchaient les rescans Git de fond puis des crashs répétés de Git 2.54 en `0xc00000fd`. Exclusion durable commit `13717755` ; sortie enfant en fichier, un composant par processus, concurrence bornée, reprise par manifeste.
- Production terminée : 63 nouveaux jobs/runs vérifiés + CHMB1/WQLS0 scellés ; manifeste familial SHA `617DD673CBB1566CFE9BFF54AB084565E62CE2BCB856888ECD1B097A397F75E3`. Montée bornée 1→2→4→8→16→23 workers ; pic contrôlé GPU 81 %, VRAM 19,0/32,6 Go, CPU 40 %, RAM 55 %, 44 °C. Catalogue offline vérifié : job SHA `88741BB909F27DA36F693CDB32E4A790E3D7253E1F03753EECA9B363432A6BDC`, 69 remplacements dont 65 pour `0x6100`, génération `CB24B195ABABF62F29DAFE1543A178EC52F692789A9477B8E67390EF3E1F514D`, catalogue SHA `EFCD5763901262458ED7E4154E3C2B24E37C4F093022B66B4594748BE269D121`. Aucune installation/release.
- Installation/QA P8 : transaction `20260914T200432.6846015Z-9503bd1f85eb47f6beb79f427389255b`, génération/catalogue `CB24B195...`/`EFCD5763...`, `519/519` shards vérifiés, 65 copiés, runtime et `Nearest` inchangés. Session `22:09:07–22:09:31` : catalogue `195/504/519`, deux instances `0x6100`, `199/199` événements avec remplacement, zéro erreur/critique ; repli natif initial unique avant chargement à la demande puis récupération. Minsc (`CHMB3+WQLS2`) et Anomen (`CHMB3+WQLWH+WQLJ6+WQLC3`) validés par l'utilisateur. Décision `sprite/index/qa-decisions/playable-characters/2026-09-14-accepted-6100-minsc-anomen-reboutcx-p8-x2-v1.json` ; les 60 autres appartenances installées ne sont pas déclarées individuellement vues ingame.
