...
Pour conserver plusieurs Characters simultanément ingame, produire chaque agrégat avec ce runbook,
puis l'ajouter au job cumulatif selon
[`SPRITE_UPSCALE_CATALOG_RUNBOOK.md`](SPRITE_UPSCALE_CATALOG_RUNBOOK.md). Ne pas remplacer
l'installation existante par une succession d'installations standard.

## Validation de référence

Référence ingame du 25 août 2026 :

- job : `sprite/families/playable-characters/6110-human-female-fighter/family-runs/complete-xn-xbr2x/jobs/human-female-fighter-complete-xn-xbr2x.json` ;
...
$characterRoot = 'sprite/families/playable-characters/<animation-id>-<character-type>'
$seedJob = "$characterRoot/<body-prefix>/jobs/$jobStem-body1-xbr2x.json"
$aggregateJob = "$characterRoot/family-runs/<aggregate>/jobs/$jobStem-complete-xn-xbr2x.json"
```
...
Le générateur publie chaque corps ou équipement dans son workspace unitaire sous `$characterRoot`,
puis publie l'agrégat en dernier sous `family-runs`. Il refuse un `$characterRoot` extérieur à
`sprite/families/playable-characters/<animation-id>-<character-type>` et ne réécrit aucun membre
existant.
...
- inscrit dans `qa.required_bam_prefixes` tous les corps et un représentant par type d'équipement.
...
Pour une installation cumulative, arrêter ici le chemin standard. Créer un append immuable avec
`generate_sprite_family_append.py catalog-append --member-job $aggregateJob`. Le générateur
conserve exactement `job_id` et `run_dir`, ajoute un seul ID et transfère le contrat
`qa.required_bam_prefixes`. Ne jamais modifier le job consigné par l'état actif.

### 4. Installer pour QA

Cette section concerne une QA isolée de l'agrégat. Fermer `BaldurReal`, `Baldur` et
`InfinityLoader`, puis restaurer tout autre test sprite actif. Pour une QA cumulative, utiliser
exclusivement l'installation catalogue ; ne pas exécuter cette commande standard.
...
Appliquer la politique de QA ingame de [`sprite/README.md`](../../README.md#politique-de-qa-ingame-pilotée).
Sans autorisation explicite de contrôle interactif, demander à l'utilisateur de lancer
`InfinityLoader.exe`, puis de tester :

- chaque variante corporelle/armure listée dans `qa.required_bam_prefixes` ;
- les ITM représentatifs listés dans `qa.required_items` ;
...
tous les préfixes requis. Cette décision reste distincte du manifeste de release. À la fin d'une tâche qui
produit du contenu `validated-installed`, appliquer la question obligatoire d'`AGENTS.md` ; sans ce
statut et sans accord affirmatif, ne rien intégrer.
