...
- Aucun script, runner ou installateur ne lance le jeu automatiquement. Appliquer la politique de
  QA ingame de [`sprite/README.md`](../../README.md#politique-de-qa-ingame-pilotée).
...
Jobs de référence :
`sprite/families/monster-icewind/e4xx-goblins/e400-mgo1-goblin-axe/legacy-workspace/jobs/goblin-mgo1-xbr2x.json`,
`sprite/families/playable-characters/6110-human-female-fighter/chfb1/jobs/human-female-fighter-chfb1-xbr2x.json`
et
`sprite/families/playable-characters/6110-human-female-fighter/family-runs/complete-xn-xbr2x/jobs/human-female-fighter-complete-xn-xbr2x.json`.
Point d'entrée agent : [`sprite/README.md`](../../README.md)
Contrat xBR : [`UPSCALE_XBR2X.md`](UPSCALE_XBR2X.md)  
Inventaire global : [`sprite/index/README.md`](../../index/README.md)

Dans les commandes suivantes, `$job` désigne un descripteur physique trouvé dans
`sprite/index/sprite-layout.json`. Ne jamais créer un nouveau job sous `sprite/jobs/` : ce chemin
est réservé aux références historiques scellées.
...
python pipeline/scripts/run_creature_sprite_x2.py new-character-job `
  --job sprite/families/playable-characters/<animation-id>-<character-type>/<body-prefix>/jobs/<job>-xbr2x.json `
...
python pipeline/scripts/run_creature_sprite_x2.py new-character-equipment-job `
  --job sprite/families/playable-characters/<animation-id>-<character-type>/<item-resref>-<bam-prefix>/jobs/<job>-xbr2x.json `
  --template-job sprite/families/playable-characters/6110-human-female-fighter/chfb1/jobs/human-female-fighter-chfb1-xbr2x.json `
  --ids-symbol <SYMBOLE_ANIMATE_IDS> `
  --animation-id 0xFFFF `
...
Le générateur historique `generate_character_complete_x2_jobs.py` n'est pas encore adapté au
modèle « un job par workspace » : il écrit tous les membres dans un même dossier `--jobs-dir`.
Ne pas l'utiliser pour une nouvelle famille organisée. Conserver les commandes ci-dessous comme
provenance des agrégats `0x6102` et `0x6110`; faire évoluer ce générateur et ses tests avant de
lancer une nouvelle production Character complète.
...
python pipeline/scripts/generate_character_complete_x2_jobs.py `
  --animation-id 0xFFFF `
  --template-job sprite/families/playable-characters/<animation-id>-<character-type>/<unique-sprite>/jobs/<membre-existant>-xbr2x.json `
  --job-stem <character> `
  --aggregate-job sprite/families/playable-characters/<animation-id>-<character-type>/family-runs/<aggregate>/jobs/<character>-complete-xbr2x.json
```
...
Utiliser `generate_sprite_family_append.py layout`, puis `member`, selon
[`SPRITE_FAMILY_CATALOG_APPEND_PIPELINE.md`](SPRITE_FAMILY_CATALOG_APPEND_PIPELINE.md). Le
générateur place le job sous
`sprite/families/monster-icewind/<code-famille>/<animation-id>-<bam-prefix>-<mob>/jobs/` et fixe
ses chemins `source_dir`, `run_dir` et `engine_build`. Utiliser
`monster-icewind-bg2ee-2.7.3.0` uniquement pour une animation `0xE000..0xEFFF`.
...
```powershell
python pipeline/scripts/run_creature_sprite_x2.py prepare `
  --job sprite/families/playable-characters/<animation-id>-<character-type>/family-runs/<aggregate>/jobs/<character>-character-set-xbr2x.json `
  --resume
```
...
`sprite/families/playable-characters/6110-human-female-fighter/family-runs/complete-xn-xbr2x/jobs/human-female-fighter-complete-xn-xbr2x.json` : elle contient toutes les familles avec
...
Exception : pour maintenir plusieurs animations dans une installation unique, ne pas contourner
ce gate et ne pas enchaîner les installateurs standard. Utiliser le job et la transaction de
catalogue définis dans
[`SPRITE_UPSCALE_CATALOG_RUNBOOK.md`](SPRITE_UPSCALE_CATALOG_RUNBOOK.md). Le catalogue accepte
seulement un import initial exact ou un append strict de contenu déjà possédé.
...
Appliquer la politique de [`sprite/README.md`](../../README.md#politique-de-qa-ingame-pilotée). Sans autorisation
explicite de contrôle interactif, demander à l’utilisateur :
...
- Pour un catalogue, appliquer ses limites propres, vérifier la sentinelle
  `animation_id=0xFFFF` du shard catalogue (V5 courant, V3 historique), les
  relations animation/composants, tous les digests/SHA/CRC et l'append strict. Ne jamais interpréter
  un shard content-addressé comme un registre autonome.
